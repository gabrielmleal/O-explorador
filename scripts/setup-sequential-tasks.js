    ...
    if (parentIssue) {
      try {
        await github.rest.issues.createComment({
          ...
          body: '❌ **Sequential task decomposition failed** - Claude Code was unable to generate the tasks.json file. Please check the context input and try again.'
        });
      } catch (commentError) {
    ...
    if (parentIssue) {
      try {
        await github.rest.issues.createComment({
          ...
          repo: context.repo.repo,
          issue_number: parseInt(parentIssue),
          body: `❌ **Sequential task decomposition failed** - tasks.json file is invalid: ${error.message}`
        });
      } catch (commentError) {
        console.log('Failed to create error comment:', commentError.message);
      }
  ...
  if (parentIssue) {
    try {
      ...
      await github.rest.issues.createComment({
        ...
        repo: context.repo.repo,
        issue_number: parseInt(parentIssue),
        body: `## 🚀 Sequential Task Execution Started
...
**Execution Strategy:**
- ✅ Tasks will execute **one at a time** in order
- 🔗 Each task builds on the changes from the previous task
- 📝 Each task will create a PR that stacks on the previous task's branch
